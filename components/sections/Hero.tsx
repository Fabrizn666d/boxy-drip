...
export function Hero() {
  ...
  const touch = useMediaQuery("(pointer: coarse), (hover: none)");
  const heroRef = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({ target: heroRef, offset: ["start start", "end start"] });
  const foregroundY = useTransform(scrollYProgress, [0, 1], [0, reducedMotion || touch ? 0 : -28]);
  ...
  return (
    <section
      ...
      onPointerMove={(event) => {
        if (reducedMotion || touch || event.pointerType === "touch") return;
    ...
    >
      <motion.div className="bd-hero-background" aria-hidden="true" initial={enter({ scale: 1.035 })} animate={{ scale: 1 }} transition={{ duration: touch ? .65 : 1.2, ease }}>
      ...
      <motion.div className="bd-hero-art" aria-hidden="true" initial={enter({ opacity: 0, x: 80, scale: .96 })} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ delay: .12, duration: touch ? .7 : 1.05, ease }}>
        <motion.div className="bd-hero-art-parallax" style={{ x: touch ? 0 : artX, y: touch ? 0 : combinedY }}><Image src="/boxy/hero-products-alien.png" alt="" fill loading="eager" fetchPriority="high" sizes="(max-width: 760px) 120vw, (max-width: 1100px) 72vw, 56vw" /></motion.div>
      </motion.div>
      ...
      <div className="bd-hero-inner">
        <div className="bd-hero-copy">
          ...
          <motion.h1 id="hero-title" initial={enter({ opacity: 0, y: 30 })} animate={{ opacity: 1, y: 0 }} transition={{ delay: .26, duration: touch ? .7 : .88, ease }}>
          ...
          <div className="bd-hero-foreground-space" aria-hidden="true" />
