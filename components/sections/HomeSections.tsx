import { ArrowRight, ArrowUpRight, Clock3, CreditCard, Headphones, MapPin, PackageCheck, RefreshCcw, ShieldCheck } from "lucide-react";
...
export function HomeSections() {
  return <>
    ...
    <section className="store-section beta-shell"><div className="store-copy"><span>04 / Find us</span><h2>Visítanos<br /><em>en tienda.</em></h2><h3>{STORE_LOCATION.name}</h3><p><MapPin /> {STORE_LOCATION.address || "Dirección pendiente de configuración"}<br />{STORE_LOCATION.district ? `${STORE_LOCATION.district}, ` : ""}{STORE_LOCATION.city}</p><p><Clock3 /> {STORE_LOCATION.schedule}</p><div>{STORE_LOCATION.mapsUrl ? <a href={STORE_LOCATION.mapsUrl} target="_blank" rel="noreferrer">Cómo llegar <ArrowUpRight /></a> : <span className="disabled-store-link">Mapa pendiente</span>}<a href={`https://wa.me/${SITE_CONFIG.whatsapp}`} target="_blank" rel="noreferrer">Escríbenos <ArrowUpRight /></a></div></div><div className="store-map-placeholder"><MapPin /><strong>Boxy Drip · Ica</strong><p>El mapa se activará al configurar la dirección oficial.</p></div></section>
